...
func (c *runitCollector) Update(ch chan<- prometheus.Metric) error {
	...
	for _, service := range services {
		...
		if err != nil {
			_ = level.Debug(c.logger).Log("msg", "Couldn't get status", "service", service.Name, "err", err)
			continue
		}

		_ = level.Debug(c.logger).Log("msg", "duration", "service", service.Name, "status", status.State, "pid", status.Pid, "duration_seconds", status.Duration)
